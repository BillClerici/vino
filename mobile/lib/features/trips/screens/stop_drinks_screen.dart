...
class _StopDrinksScreenState extends ConsumerState<StopDrinksScreen> {
  ...
  Future<void> _addDrink({Map<String, dynamic>? prefill}) async {
    ...
    try {
      ...
      final menuItemImage = prefill?['_menu_item_image'] as String? ?? '';
      setState(() {
        _drinks.add({
          ...result,
          'id': drinkId,
  ...
  Future<void> _addFromMenu(Map<String, dynamic> menuItem) async {
    final prefill = <String, dynamic>{
      ...
      '_menu_item_image': menuItem['image_url'] ?? '',
    };
    ...
    if (drinkId.isNotEmpty) {
      try {
        ...
        setState(() => _drinks[index] = {
              ...
              if (photoUrl != null && photoUrl.isNotEmpty) 'photo': photoUrl,
              'menu_item_image': drink['menu_item_image'] ?? '',
            });
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
...
class _DetailedDrinkCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final vintage = drink['wine_vintage'];
    ...
    final menuItemImage = drink['menu_item_image'] as String? ?? '';
    ...
    return Card(
      ...
      child: Column(
        ...
        children: [
          ...
          GestureDetector(
            onTap: hasImage ? () => _showFullImage(context, drinkImage) : null,
            child: Stack(
              children: [
                SizedBox(
                  height: hasImage ? 160 : 80,
                  width: double.infinity,
                  child: hasImage
                      ? Image.network(
                          resolvedImage,
                ...
                // Top-right badges: favorite + purchased
                Positioned(
                  top: 8,
                  right: 8,
                  child: Row(
  ...
  Widget _gradientHeader(ColorScheme cs) {
    return Container(
      ...
      child: Center(
        child: Icon(
          placeType == 'brewery' ? Icons.sports_bar : Icons.wine_bar_rounded,
          size: 36,
